...
class AslrCheck(interfaces.plugins.PluginInterface):
    ...
    def _generator(self, procs):
        ...
        for proc in procs:
            ...
            try:
                ...
                pe_data = io.BytesIO()
                for offset, data in dos.reconstruct():
                    pe_data.seek(offset)
                    pe_data.write(data)

                pe = pefile.PE(data=pe_data.getvalue())
                aslr = check_aslr(pe)
                ...
                vollog.debug(f"Skipping {procname}: {e}")
    ...
    def run(self):
        procs = pslist.PsList.list_processes(
            ...
            filter_func=self.create_pid_filter(self.config.get("pid"))
        )

        return renderers.TreeGrid(
            [("PID", int), ("Filename", str), ("Base", format_hints.Hex), ("ASLR", bool)],
